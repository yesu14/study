...
str_input_buy_msg = "\033[1;36mPlease enter the product number your want to buy:\033[0m(if want return login menu enter \033[31mlogin\033[0m)"
str_order_add = "a"  # 添加
str_order_modify = "b"  # 修改
str_order_delete = "c"  # 删除
str_order_complete = "\033[32;1mExecution completed!\033[0m"

str_order_wrong = "\033[31;1mSyntax input Error,please try again later!\033[0m"
            ...
            while swich_seller_item:
                ...
                if order_list[0] == str_order_add and len(order_list) == 3:
                    with open("items.txt", "a+", encoding="utf-8") as items_file:
                        items_file.write(order_list[1] + "," + order_list[2]+"\n")
                        items_file.flush()
                        print("\033[33;1mItem add completed!","Item:",order_list[1],"Price:",order_list[2],"\033[0m")
                # 修改操作
                elif order_list[0] == str_order_modify and len(order_list) == 4:
                    with open("items.txt", "r", encoding="utf-8") as items_file:
                        with open("items.bak","w",encoding="utf-8") as new_file:
                            count = 0
                            for line in items_file:
                                count += 1
                                if str(count) == order_list[1]:
                                    _item = line.strip("\n").split(",")
                                    line = order_list[2] + "," + order_list[3] + "\n"
                                new_file.write(line)
                                new_file.flush()
                    os.replace("items.bak","items.txt")
                    print("\033[33;1mItem modify completed!\033[0m")
                # 删除操作
                    ...
                    with open("items.txt", "r", encoding="utf-8") as items_file:
                        with open("items.bak","w",encoding="utf-8") as new_file:
                            ...
                            for line in items_file:
                                ...
                                new_file.flush()
                    os.replace("items.bak", "items.txt")
                    print("\033[41;1mItem delete completed!\033[0m")
...
def show_shopping_list(username):
    with open("shopping_list.txt", "r", encoding="utf-8") as shopping_list:
        for line in shopping_list:
            _line = line.strip().split(",", 1)
            if username == _line[0]:
                print("Your shopping list :")
                list = (_line[1].replace("[","").replace("]","")).split(",")
                for i, s_list in enumerate(list):
                    print(i+1, ".", s_list.strip())
                break

#修改用户文件
def modify_user_file(username,balance):
    with open("user.txt","r",encoding="utf-8") as user_file:
        with open("user.bak","w",encoding="utf-8") as new_file:
            for line in user_file:
                _line = line.strip("\n").split(",")
                if username == _line[0]:
                    line = _line[0] + ","+_line[1] +","+ str(balance) + "\n"
                new_file.write(line)
                new_file.flush()
    os.replace("user.bak","user.txt")
#修改购买列表
def modify_shopping_list(username, item):
    with open("shopping_list.txt", "r", encoding="utf-8") as shopping_list_file:
        with open("shopping_list.bak", "w", encoding="utf-8") as new_file:
            for line in shopping_list_file:
                ...
                if username == _line[0]:
                    ...
                    items = (line.strip().replace("[","").replace("]","")).replace(_line[0] + ",", "")
                    _list = items.replace(" ","").split(",")
                    _list.append(item)
                    line = _line[0] + ",[" + str(_list).replace(" ","").replace("[","").replace("]","").replace("'","")+"]\n"
                new_file.write(line)
                new_file.flush()
...
def user_login():
    ...
    while swich_buyer_login:
        ...
        if result != 0 and result[0] == 1:
            ...
            show_shopping_list(username) #展示购物列表
            while swich_buyer_item:
                        ...
                        for line in items_file:
                            ...
                            if count == int(shopping_input):
                                if balance >= int(_line[1]): #余额大于等于商品价格时
                                    ...
                                    print(str_balance_not_enough %(balance))
                else:
                    print(str_wrong_msg)
